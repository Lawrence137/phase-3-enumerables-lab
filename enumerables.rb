...
def get_spicy_food_by_cuisine(spicy_foods, cuisine)
  spicy_foods.find { |item| item[:cuisine] == cuisine }
end
...
def print_spiciest_foods(spicy_foods)
  spiciest_foods(spicy_foods).each do |item|
...
def average_heat_level(spicy_foods)
  total = spicy_foods.reduce(0) { |sum, item| sum + item[:heat_level] }
  (total / spicy_foods.length).to_i
end
